  ...
  const fetchEmployees = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5002/laundry/api/users/all",
        {
          headers: {
    ...
    if (window.confirm("Are you sure you want to delete this employee?")) {
      try {
        const response = await axios.delete(
          `http://localhost:5002/laundry/api/deleteUser/${userId}`
        );
        if (response.data.success) {
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-3 items-center">
          ...
          <Link to="/admin-dashboard/register-employee" className="bg-blue-500 text-white rounded-lg flex items-center gap-2 px-4 py-2 font-semibold">
            Add Employee
            <FaPlus className=""/>
          </Link>
        </div>
